...
const manipulateProjects = (projects) => {
  let projectsHTML = projects.map((projectItem, i) => {
    return i % 2 === 0
      ? `<div class="ProjectContainer relative flex mx-auto justify-between items-center ${!projectItem.image.device.toLowerCase() === "mobile" ? "py-0" : "py-10"}">
                <div class="ProjectImage w-[48%] relative flex justify-start ">
                    <div class="relative hover:scale-110 ease-in-out duration-200 ">
                        <img class="${projectItem.image.device.toLowerCase()} aspect-3/2 object-cover" height="auto" src="${projectItem.image.url
      }" alt="Loading..." />
                    </div>
                </div>

                <div class="circle absolute left-1/2 -translate-x-2 top-50" style="background:${projectItem.headingColor
      }">
                    <div class="white-circle"></div>
                </div>
                <div
                    class="absolute extnsn h-[1px] w-[35%] -z-20 flex items-center justify-start right-1/2" style="background:${projectItem.headingColor
      }">

                </div>

                <div class="cntnt w-[48%] min-h-64 px-[2%]">
                    <div class="heading text-4xl my-2 font-semibold handlee custom-font tracking-[2px]" style="color:${projectItem.headingColor
      }">
                        ${projectItem.title}
                    </div>
                    <div class="mini-description text-xl text-[${projectItem.headingColor
      }] mb-3 custom-font tracking-[1px] font-bold" style="color:${projectItem.headingColor
      }">
      ...
      : `<div class="ProjectContainer relative flex mx-auto justify-between items-center py-10">
        ...
        .join("")}
                ...
                <div
                    class="absolute extnsn h-[1px] w-[35%] -z-10 flex items-center justify-start left-1/2" style="background:${projectItem.headingColor
      }">
                </div>
                <!-- </div> -->

                <div class="ProjectImage w-[48%] flex justify-end">
                    <img class="hover:scale-110 ease-in-out duration-200 ${projectItem.image.device.toLowerCase()} aspect-3/2 object-cover" height="auto" src="${projectItem.image.url
      }"
                        alt="Loading..." />
                </div>
